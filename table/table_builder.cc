...
struct TableBuilder::Rep {
  ...
  std::string last_key; // Rep保存的是有序 key-value 因此每次添加操作需要保存最大key
  int64_t num_entries; // key-value 数目
  ...
  // 直到我们看到新的一个data block我们才会为这个data block创建一个新的index entry
  // 在这个index block中允许我们使用更短的key。例如在keys  "the quick brown fox" 和 "the who"
  // 之间确定边界。我们可以使用 "the r"作为边界，因为它("the r")大于等于当前index block中所有entry
  // 且它小于后续blocks中所有entryies。
  // 具体是什么意思呢?
  // 1、我们不会轻易创建一个新的index_block，除非有一个新的data_block创建才会重新生成index_block
  // 2、方便我们使用更短字符串作为keys，例如: 以"the r"作为边界,当大于"the r"在一个data block,
  //    小于的在另外一个data block中
